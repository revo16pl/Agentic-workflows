...
Output: runtime/convert-for-ai/output/{stem}.pdf

Usage:
    python3 workflows/convert-for-ai/scripts/build_document.py "runtime/tmp/convert-for-ai/WS-06 Milestones"
    python3 workflows/convert-for-ai/scripts/build_document.py "runtime/tmp/convert-for-ai/WS-06 Milestones" "custom/output/dir"
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 workflows/convert-for-ai/scripts/build_document.py <tmp_dir> [output_dir]")
        sys.exit(1)

    tmp_dir = Path(sys.argv[1]).resolve()
    output_dir = Path(sys.argv[2]).resolve() if len(sys.argv) > 2 else (PROJECT_ROOT / "runtime" / "convert-for-ai" / "output")
    ...
    if not extracted_path.exists():
        ...
        print("Run Step 1 first: python3 workflows/convert-for-ai/scripts/convert_for_ai.py")
        sys.exit(1)
