...
ARTICLE_SCRIPTS_DIR = PROJECT_ROOT / "workflows" / "agentic-articles" / "scripts"
if str(ARTICLE_SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(ARTICLE_SCRIPTS_DIR))
...
PLANNING_ROOT = PROJECT_ROOT / "runtime" / "agentic-articles" / "planning"
...
def run_pipeline_steps(sprint_dir: Path) -> tuple[bool, list[str]]:
    commands = [
        ["python3", str(PROJECT_ROOT / "workflows" / "content-planning" / "scripts" / "content_planning_cluster.py"), "--sprint", str(sprint_dir)],
        ["python3", str(PROJECT_ROOT / "workflows" / "content-planning" / "scripts" / "content_planning_gate.py"), "--sprint", str(sprint_dir)],
        ["python3", str(PROJECT_ROOT / "workflows" / "content-planning" / "scripts" / "content_planning_to_queue.py"), "--sprint", str(sprint_dir)],
    ]
    outputs: list[str] = []
    for cmd in commands:
...
def main() -> int:
    ...
    summary = {
        ...
        "next_step": (
            f"python3 workflows/agentic-articles/scripts/prepare_article_from_queue.py --queue-row-id '{queue_row}'"
            if queue_row
