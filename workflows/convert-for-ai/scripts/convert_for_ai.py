...
Usage:
    python3 workflows/convert-for-ai/scripts/convert_for_ai.py [input_folder]

Default input_folder: runtime/convert-for-ai/input
Output: runtime/tmp/convert-for-ai/{stem}/slides/*.png  +  runtime/tmp/convert-for-ai/{stem}/extracted.json
"""
...
def main():
    input_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else PROJECT_ROOT / "runtime" / "convert-for-ai" / "input"
    ...
    tmp_root = PROJECT_ROOT / "runtime" / "tmp" / "convert-for-ai"
    ...
    if tmp_dirs:
        ...
        for d in tmp_dirs:
            print(f"  python3 workflows/convert-for-ai/scripts/build_document.py \"{d}\"")
