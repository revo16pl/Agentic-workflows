...
Użytkownik wrzuca plik do `runtime/convert-for-ai/input/` i daje jeden prompt.
Claude wykonuje CAŁY pipeline automatycznie:

```
1. python3 workflows/convert-for-ai/scripts/convert_for_ai.py
...
2. Claude czyta każdy PNG + extracted.json
   ...
   → zapisuje runtime/tmp/convert-for-ai/{stem}/descriptions.json

3. python3 workflows/convert-for-ai/scripts/build_document.py "runtime/tmp/convert-for-ai/{stem}"
   → PDF w runtime/convert-for-ai/output/
```
...
runtime/tmp/convert-for-ai/{stem}/       ← pliki pośrednie (można usunąć po konwersji)
  slides/
...
- `workflows/convert-for-ai/scripts/convert_for_ai.py` — Krok 1
- `workflows/convert-for-ai/scripts/build_document.py` — Krok 3
