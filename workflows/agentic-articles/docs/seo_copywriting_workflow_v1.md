...
- Dodano URL bootstrap dla service pages:
  - `python3 workflows/agentic-articles/scripts/prepare_service_page_from_url.py --url ... --company ...`
- Dodano extractor kontekstu strony:
  - `workflows/agentic-articles/scripts/extract_service_page_context.py` (Playwright first, curl fallback).
- Dodano profile-aware walidacje:
  - `python3 workflows/agentic-articles/scripts/article_workflow_validate.py --content-profile service_page`
  - walidacja sekcji: `Subheading`, `Opis`, `Najważniejsze informacje`.
- Dodano profile-aware export:
  - `python3 workflows/agentic-articles/scripts/export_to_gdocs.py --content-profile service_page`.
...
- Dodano machine-readable profile firmy:
  - `workflows/agentic-articles/docs/company_context_profiles.yaml`,
  - resolver: `workflows/agentic-articles/scripts/company_profile_resolver.py`,
...
- Dodano osobny Workflow A (bi-weekly content planning sprint) z artefaktami:
  - `runtime/agentic-articles/planning/{YYYY-MM-DD}_sprint/`
...
- Workflow B startuje wyłącznie z `RUN_QUEUE` przez:
  - `python3 workflows/agentic-articles/scripts/prepare_article_from_queue.py --queue-row-id \"rq-0001\"`
...
`runtime/agentic-articles/workspace/{YYYY-MM-DD}_{slug}/`
...
Dzialania:
- uruchom `workflows/agentic-articles/scripts/check_polish_language.py` na `article_draft_v2.md` (lub przygotowanym draftcie roboczym),
...
Dzialania:
- uruchom `workflows/agentic-articles/scripts/check_polish_naturalness.py`,
...
Dzialania:
- uruchom `workflows/agentic-articles/scripts/check_polish_fluency_ml.py` (pre-trained model, bez treningu lokalnego),
...
Dzialania:
- uruchom `workflows/agentic-articles/scripts/check_humanization.py` na finalnym drafcie roboczym,
...
Dzialania:
- uruchom `workflows/agentic-articles/scripts/check_workflow_context.py`,
...
- pre-trained Polish LM signal (pseudo-perplexity) przez `workflows/agentic-articles/scripts/check_polish_fluency_ml.py`,
...
- wymagane skills sa zadeklarowane i uzyte w `run_context.md`:
  - `content-strategy`
  - `copywriting`
  - `copy-editing`
  - `seo-audit`
